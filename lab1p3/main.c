...
// expect string to be at least dd:dd:dd
    ...
    while(testing[i] != '\0'){
        if(testing[i] != timeString[i]){
            noError = 1; // no stdout in MPLABX, hence using LEDs to indicate success or failure
            LATDbits.LATD0 = 1; // red LED illuminates for failure
            break;
        }
        i++;
    }
    if(noError == 0){
        LATDbits.LATD2 = 1; // green LED illuminates for success
    }
